---
title: "Next.jsのMiddlewareを使ってvercelステージング環境を構築しよう"
emoji: "🤖"
...
今回は`[本番環境のドメイン]-develop.vercel.app`のように追加しました。
👇
...
そのため、先程の内容を追加することで独自ドメインを設定した Preview 環境にも`X-Robots-Tag: noindex`を設定でき、検索エンジンにインデックスされないようにすることができます。
追加後に、curl コマンドをもう一度実行してみると、`X-Robots-Tag: noindex`が設定されていることが確認できました。
👇

```bash:X-Robots-Tagが設定されている
curl -I https://[設定したドメイン]
HTTP/2 200
accept-ranges: bytes
access-control-allow-origin: *
...
以上でセキュリティを考慮したステージング環境構築が完了です 🎉

## 実装完了

以上が SEO の問題やセキュリティを考慮したステージング環境構築が完了です ✨
